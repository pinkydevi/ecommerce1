...
class ProductController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            $product="";
              $query=DB::table('products')->leftJoin('categories','products.category_id','categories.id')
                ...
                if ($request->product_slider==1) {
                    $query->where('products.product_slider',1);
                }
                if ($request->product_slider==0) {
                    $query->where('products.product_slider',0);
                }
            ...
            return DataTables::of($product)
                    ...
                    ->editColumn('product_slider',function($row){
                        if ($row->product_slider==1) {
                            return '<a href="#" data-id="'.$row->id.'" class="deactive_product_slider"><i class="fas fa-thumbs-down text-danger"></i> <span class="badge badge-success">active</span> </a>';
                        }else{
                            return '<a href="#" data-id="'.$row->id.'" class="active_product_slider"><i class="fas fa-thumbs-up text-danger"></i> <span class="badge badge-danger">deactive</span> </a>';
                        }
                    })
                    ->addColumn('action', function($row){
                        $actionbtn='
                        <a href="#" class="btn btn-primary btn-sm quick_view" id="'.$row->id.'" data-toggle="modal" data-target="#exampleModalCenter"><i class="fas fa-eye"></i></a>
                    ...
                    ->rawColumns(['action','category_name','subcategory_name','brand_name','thumbnail','featured','today_deal','status','product_slider'])
                    ->make(true);       
        }
    ...
    public function store(Request $request)
    {
       ...
       if ($request->thumbnail) {
            ...
            $img = $img->cover(540,600,'top');
            $img->save('files/product/'.$photoname);
            $data['thumbnail']='files/product/'.$photoname;   // files/product/plus-point.jpg
        }
       ...
       //multiple images
        ...
        if($request->hasFile('images')){
            ...
            foreach ($request->file('images') as $key => $image) {
                ...
                $img = $img->cover(540,600,'top');
                $img->save('files/product/'.$imageName);
    ...
    //not status
    public function not_product_slider($id)
    {
        DB::table('products')->where('id',$id)->update(['product_slider'=>0]);
        return response()->json('Product Deactive');
    }

    //active staus
    public function active_product_slider($id)
    {
        DB::table('products')->where('id',$id)->update(['product_slider'=>1]);
        return response()->json('Product Activated');
    }
